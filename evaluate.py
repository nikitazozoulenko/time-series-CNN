...
def main():
    ...
    for i in range(0, 100001, 1000):
        ...
        evaluate_acc(batch_size, single, test_data, i, single_test_loss, single_test_acc)
        double.load_state_dict(torch.load("savedir/double_it"+str(i//1000)+"k.pth"))
        evaluate_acc(batch_size, double, test_data, i, double_test_loss, double_test_acc)
        radical.load_state_dict(torch.load("savedir/radical_it"+str(i//1000)+"k.pth"))
        evaluate_acc(batch_size, radical, test_data, i, radical_test_loss, radical_test_acc)
